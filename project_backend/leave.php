...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['submit_leave'])) {
  ...
  if ($enrollment_no && $from_date && $to_date && $reason) {
    ...
    if ($interval > 0) {
      ...
      $stmt->execute();
      echo "<script>alert('Leave recorded successfully.'); window.location.href='leave.php?enrollment_no=$enrollment_no';</script>";
    } else {
...
<head>
  <meta charset="UTF-8">
  <title>Student Leave Entry</title>
  <style>
    body {
...
<div class="container">
  ...
  <form method="POST">
    <?php if (!$readonly): ?>
      <label for="enrollment_no">Student</label>
    ...
    <textarea name="reason" rows="4" required placeholder="e.g. Sick, Family Function, Emergency, etc."></textarea>
  ...
  <?php elseif ($selected_enrollment): ?>
    <p style="text-align:center; color:#888;">No leave records found for <?= htmlspecialchars($student_name) ?>.</p>
  <?php endif; ?>
</div>
