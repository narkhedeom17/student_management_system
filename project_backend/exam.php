...
if (!$selected_enrollment) die("Enrollment number not provided in the URL.");
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['submit_marks'])) {
  ...
  if ($exam_name && $exam_date && $subject && $total_marks > 0 && $marks_obtained >= 0) {
    ...
    $stmt->execute();
    echo "<script>alert('Marks added successfully.'); window.location.href='exam.php?enrollment_no=$selected_enrollment';</script>";
    exit;
  } else {
...
<div class="container">
  <h2>Enter Marks for <?= htmlspecialchars($student_name) ?></h2>

  <form method="POST">
    <label for="exam_name">Exam Name</label>
    <input type="text" name="exam_name" required placeholder="e.g. Class Test, Unit Test">
  ...
  <?php else: ?>
    <p style="text-align:center;color:#888;">No exam records found.</p>
  <?php endif; ?>
</div>
